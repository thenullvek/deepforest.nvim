local validate = require("deepforest.validate")
    ...
    for color_name, color_value in pairs(tweak_color) do
        if color_value and color_value ~= "default" then
            if validate.hexcode(color_value) then
                ...
                vim.notify(
                    "ERROR: skiping deepforest tweak color [" .. color_name .. "] because of invalid value (" .. color_value .. ")",
    ...
    for _, key in ipairs(tweak_background_keys) do
        ...
        if value and (value ~= "default") then
            if validate.hexcode_or_none(value) then
                ...
                vim.notify("ERROR: skiping deepforest tweak background [" .. key .. "] because of invalid value (" .. value .. ")", vim.log.levels.ERROR)
            end
        end
    end
end
...
---@param tweak_syntax DeepForestConfigTweakSyntax
    ...
    for _, key in ipairs(tweak_syntax_keys) do
        ...
        if value and (value ~= "default") then
            if validate.hexcode_or_none(value) then
                ...
                vim.notify("ERROR: skiping deepforest tweak syntax [" .. key .. "] because of invalid value (" .. value .. ")", vim.log.levels.ERROR)
            end
        end
    end
end
...
---@param tweak_ui DeepForestConfigTweakUI
---@param theme DeepForestTheme
---@param color DeepForestColor
